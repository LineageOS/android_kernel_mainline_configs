...
class LoadStage(Enum):
	...
	VENDOR = "vendor"
	"""
	Modules loaded with second stage init, contained in vendor.
	"""

	SYSTEM = "system"
	"""
	Modules loaded as system DLKM, contained in system.
	"""
...
def main():
	...
	with TemporaryDirectory() as temp_dir:
		...
		for line in modules_dep_path.read_text().splitlines():
			...
			# Make sure this module is known
			assert module_name in modules_order_name, \
				f"Module {module_name} not found in modules.order"

			# Make sure modules_dep doesn't contain the module already
			assert module_name not in modules_dep, \
				f"Module {module_name} already in modules.dep"

			modules_dep[module_name] = deps
